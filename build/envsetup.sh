...
function droidxremote()
{
    ...
    local DROIDX_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$DROIDX_USER" ]
    then
        git remote add droidx ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add droidx ssh://$DROIDX_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
