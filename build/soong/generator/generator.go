...
func init() {
	android.RegisterModuleType("droidx_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = droidxExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = droidxExpandVariables(ctx, dep_file)
	...
	cmd := droidxExpandVariables(ctx, String(g.properties.Cmd))
